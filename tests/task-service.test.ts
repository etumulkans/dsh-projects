...
interface Fixture {
  ...
  /** Phase 8: the fixture's artifact service, when `overrides.artifactFactory` was given. */
  artifactService?: ProjectArtifactService | undefined
}
...
async function fixture(overrides: {
  ...
  /** Phase 8: build the artifact service (absent = no artifact service). */
  ...
  const artifactService = overrides.artifactFactory === undefined ? undefined : overrides.artifactFactory(ctx, catalog, runService)
  if (artifactService !== undefined) artifactService.start()
  ...
  return { ctx, runService, planService, taskService, held, emit, runId: run.id, catalog, memory, approvalService, artifactService }
}
...
describe('ProjectTaskService Phase 8 run/completed report trigger (spec §7)', () => {
  it('generates the final report when the run reaches a terminal phase', async () => {
    const fx = await fixture({
      artifactFactory: (ctx, catalog, runService) => new ProjectArtifactService(ctx, catalog, runService),
    })
    const artifacts = fx.artifactService!
    try {
      // Seed a run-scoped artifact so the report has a changes section to draw from.
      ...
      // Drive the run to a terminal phase; the run/completed event fires and the
      // artifact service's listener generates the final report (fire-and-forget).
      await fx.runService.transitionRun(fx.runId, 'finalizing')
      await fx.runService.transitionRun(fx.runId, 'succeeded', { resultSummary: 'done' })
      await waitFor(async () => {
        const report = artifacts.list({ runId: fx.runId }).find(a => a.kind === 'final-report')
        return report
      }, 'final report generated by the run/completed trigger')
      const report = artifacts.list({ runId: fx.runId }).find(a => a.kind === 'final-report')
      expect(report).toBeDefined()
      expect(report!.content).toContain('Goal')
      expect(report!.content).toContain('Remaining risks')
      // The run carries the artifact.created event for the final report.
      const detail = await fx.runService.runDetail(fx.runId)
      expect(detail.events.some(e => e.type === 'artifact.created')).toBe(true)
    } finally {
      artifacts.stop()
      fx.planService.stop()
      fx.taskService.stop()
